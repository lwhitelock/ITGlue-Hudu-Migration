...
if ($true -eq $allowSettingFlagsAndTypes){
    if (-not (Get-Command -Name Get-HuduFlagTypes -ErrorAction SilentlyContinue)) { 
        ...
        $flagsResult = Get-UserFlagSetup; $ObjectFlagMap = $flagsResult.ObjectFlagMap ?? @{}; $allowSettingFlagsAndTypes = $flagsResult.AllowSettingFlags ?? $false; $passwordFlagCategories = $flagsResult.passwordFlagCategories ?? $false;
    }
}                
    ...
    if ($true -eq $flagPasswordsByType){
        ...
        foreach ($category in $passwordFlagCategories) {
            if (-not [string]::IsNullOrWhiteSpace($category)) {
                ...
                $matchedpasswords | Where-Object { $_.ITGObject.attributes.'password-category-name' -ieq $category -and $_.HuduObject.id -and $_.HuduObject.id -gt 0 } | ForEach-Object {
                    Set-HuduFlag -flagTypeId $passwordFlagMap[$category].id -FlagableType "Password" -FlagableID $_.HuduObject.id
                }
            }
        }
    } else {
