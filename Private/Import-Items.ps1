        ...
        if (($importOption -eq "A") -or ($importOption -eq "S") ) {		
            ...
            foreach ($company in $CompaniesToMigrate) {
                ...
                if ($itgimport.attributes.archived) {
                    Write-Host "WARNING: $($HuduAssetName) is archived in ITGlue? '$($itgimport.attributes.archived.ToString().ToLower())' and is being set in Hudu" -ForegroundColor Magenta
                    $Null = Set-HuduAssetArchive -Id $HuduNewImport.id -CompanyId $HuduNewImport.company_id -Archive $($false -eq $itgimport.attributes.archived.ToString().ToLower())
                }
